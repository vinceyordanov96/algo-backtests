#!/usr/bin/env python3
# algo-backtests/examples/momentum_atr.py
"""
Momentum ATR Strategy Example - Complete Backtesting Workflow.
...
Usage:
    python examples/momentum_atr.py
    
Outputs:
    - outputs/strategies/momentum_atr/simulations/{ticker}/  - Simulation results
    - outputs/strategies/momentum_atr/signals/{ticker}/  - Best strategy signals
    - outputs/strategies/momentum_atr/results/{ticker}/  - Portfolio results
    - outputs/strategies/momentum_atr/plots/{ticker}/  - Performance plots
"""
...
def main():
    """Main entry point for the momentum ATR strategy example."""
    
    print("\n\n" + "=" * 80)
    print("MOMENTUM ATR STRATEGY - BACKTESTING")
    print("=" * 80)
    print()
    
    from connectors import DataFetcher, DataManager
    from simulation import SimulationRunner, SimulationConfig, StrategyType
    from simulation.config import CommonParameters, MomentumATRParameters
    ...
    TICKER = 'NVDA'
    DATA_SOURCE = 'polygon'
    STRATEGY = StrategyType.MOMENTUM_ATR
    ...
    common_params = CommonParameters(
        ...
        sizing_types=['vol_target'],    # one or all of ['vol_target', 'kelly', 'kelly_vol_blend']
    ...
    momentum_params = MomentumATRParameters(
        atr_multipliers=[1.5, 1.75],
        atr_periods=[14, 20]
    )
    ...
    simulation_config = SimulationConfig(
        ...
        momentum_atr=momentum_params,
        n_workers=None  # Auto-detect
    )
    
    print(f"  ✓ Ticker: {TICKER}")
    print(f"  ✓ Strategy: {STRATEGY.value}")
    print(f"  ✓ ATR multipliers: {momentum_params.atr_multipliers}")
    print(f"  ✓ ATR periods: {momentum_params.atr_periods}")
    ...
    print(f"  ✓ Sizing type: {common_params.sizing_types}")
    if 'kelly' in common_params.sizing_types:
        print(f"  ✓ Kelly fractions: {common_params.kelly_fractions}")
        print(f"  ✓ Kelly lookbacks: {common_params.kelly_lookbacks}")
        print(f"  ✓ Kelly min trades: {common_params.kelly_min_trades}")
        if 'kelly_vol_blend' in common_params.sizing_types:
            print(f"  ✓ Kelly vol blend weight: {common_params.kelly_vol_blend_weight}")
    ...
    if df_intraday is not None and df_daily is not None:
        df_intraday = df_intraday[df_intraday.index >= '2023-11-01']
    ...
    print(f"  - ATR Multiplier: {best_row.get('ATR Multiplier', 'N/A')}")
    print(f"  - ATR Period: {best_row.get('ATR Period', 'N/A')}")
    ...
    for output_type in ['simulations', 'signals', 'results', 'plots']:
        info = output_manager.get_output_info('momentum_atr', TICKER, output_type)
        if info:
            print(f"  - {output_type.capitalize()}: {info['filepath']}")
    
    print("\n\n" + "=" * 80)
    print("Momentum ATR Strategy Example completed successfully!")
    print("=" * 80)
