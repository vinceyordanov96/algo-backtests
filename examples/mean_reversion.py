...
def main():
    """Main entry point for the mean reversion strategy example."""
    
    print("\n\n" + "=" * 80)
    print("MEAN REVERSION STRATEGY - BACKTESTING")
    print("=" * 80)
    print()
    
    from connectors import DataFetcher, DataManager
    from simulation import SimulationRunner, SimulationConfig, StrategyType
    from simulation.config import CommonParameters, MeanReversionParameters
    ...
    # Data source: 'yfinance' (free) or 'polygon' (requires API key)
    DATA_SOURCE = 'polygon'
    ...
    mean_reversion_params = MeanReversionParameters(
        zscore_lookbacks=[20],
        n_std_uppers=[2.0],
        n_std_lowers=[2.0],
        exit_thresholds=[0.0],
    ...
    print(f"  ✓ zscore lookbacks: {mean_reversion_params.zscore_lookbacks}")
    print(f"  ✓ n_std_uppers: {mean_reversion_params.n_std_uppers}")
    print(f"  ✓ n_std_lowers: {mean_reversion_params.n_std_lowers}")
    print(f"  ✓ exit_thresholds: {mean_reversion_params.exit_thresholds}")
    ...
    print(f"  - zscore lookback: {best_row.get('zscore lookback', 'N/A')}")
    print(f"  - n_std_upper: {best_row.get('n_std_upper', 'N/A')}")
    print(f"  - n_std_lower: {best_row.get('n_std_lower', 'N/A')}")
    print(f"  - exit_threshold: {best_row.get('exit_threshold', 'N/A')}")
    ...
    for output_type in ['simulations', 'signals', 'results', 'plots']:
        info = output_manager.get_output_info('mean_reversion', TICKER, output_type)
        if info:
            print(f"  - {output_type.capitalize()}: {info['filepath']}")
    
    print("\n\n" + "=" * 80)
    print("Mean Reversion Strategy Example completed successfully!")
    print("=" * 80)
